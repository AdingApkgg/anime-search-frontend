  ...
  return (
    <div className="w-full mb-4 sm:mb-6">
      ...
      >
        ...
        <span className="text-sm font-semibold text-muted-foreground">Bangumi 匹配</span>
        <Badge variant="secondary">{bangumiList.length}</Badge>
      </motion.div>

      <motion.div
        className="flex gap-3 px-3 sm:px-4 overflow-x-auto snap-x snap-mandatory pb-2 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-primary/30 hover:scrollbar-thumb-primary/50"
        variants={container}
        initial="hidden"
        animate="show"
      >
  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden">
        ...
        {score && (
          <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 rounded-lg bg-black/70 border border-white/10">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          <h3 className="text-sm font-bold text-white line-clamp-2 mb-1.5 group-hover:text-primary transition-colors">
            {displayName}
          </h3>
      ...
      {summary && (
        <div className="hidden sm:block absolute inset-x-0 bottom-0 p-3 bg-black/90 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out">
          <p className="text-[11px] text-white/80 line-clamp-4 leading-relaxed">
            {summary}
          </p>
          {name_cn && name_cn !== name && (
            <p className="mt-2 text-[10px] text-white/50 truncate">{name}</p>
          )}
        </div>
      )}
    </motion.a>
